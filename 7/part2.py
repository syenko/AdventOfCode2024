...
can be solved by inserting + or * or concatenate operators
...
def can_solve_helper(cur, res, i):
    ...
    if cur > res:
        ...
        # try both options + or * or ||
        return can_solve_helper(cur * nums[i], res, i + 1) or can_solve_helper(cur + nums[i], res, i + 1) or helper(int(str(cur) + str(nums[i])), res, i + 1)
